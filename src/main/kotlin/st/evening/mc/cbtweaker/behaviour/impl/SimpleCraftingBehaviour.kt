...
object SimpleCraftingBehaviour : MachineBehaviour<SimpleCraftingBehaviour.State> {
    ...
    override fun loadStateFactory(machine: CraftingBlockType<*>, dto: TJson.Object): MachineStateFactory<State> {
        ...
        return factory@{ world, pos, bufGroups, components, host, oldState ->
            if (oldState != null && oldState.config === config && oldState.recipeDb === recipeDb) {
                oldState.reinit(world, pos, bufGroups, config.computeModifierState(components), host)
                return@factory oldState
            } else {
                return@factory State(
                    config, recipeDb, world, pos, bufGroups, config.computeModifierState(components), host
                )
            }
        }
    }
    ...
    override fun isActive(state: State): Boolean = state.isActive()
    ...
    class State(
        ...
        fun notifyState(newComponents: ComponentSet?) {
        ...
        private class UiState(state: ListStateComposite.Builder = ListStateComposite.Builder()) :
