...
class SingleBlockMachineBlock(val sbType: SingleBlockType<*>) :
    ...
    override fun onBlockActivated(
        ...
        world.useTileEntity<SingleBlockMachineTileEntity>(pos) {
            if (it.handleInteraction(player, hand, facing, hitX, hitY, hitZ)) return true
            world.onServer {
                player.openContainer(CbTweaker.defns.containerSingleBlockMachine, world, pos)
            }
            return true
        }
        return false
    ...
    override fun getTranslationKey(): String = "${CbtConsts.MOD_ID}.singleblock.${sbType.id}"
}
